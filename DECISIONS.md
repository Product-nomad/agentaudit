...
## 2026-04-23 — Root cause of the release-pipeline failures was missing `repository` field in package.json

After five consecutive publish failures across OIDC and token auth, switching to the automation token surfaced a `422 Unprocessable Entity` from npm's provenance validator with the actual reason: `"repository.url" is "", expected to match "https://github.com/Product-nomad/agentaudit" from provenance`. npm's provenance-validation path refuses to persist the attestation unless `package.json.repository.url` equals the repo URL that the SLSA statement vouches for. The earlier OIDC attempts likely surfaced the same failure as 404 through the trusted-publisher auth path, which is why the config-matching theory kept fitting without resolving anything. Fix was one block: add `repository`, `homepage`, `bugs` to `package.json`. *Lesson:* when a 404 persists despite apparently-correct config, distrust the error code and switch auth paths to surface the real response.

## 2026-04-23 — Fall back to automation token for npm publish; keep OIDC for provenance signing

After four consecutive publish attempts (v0.2.1–v0.2.3) all failed with `404 PUT` at the npm registry despite OIDC working through to Sigstore every time (provenance statements are visible on Rekor at logIndex 1365472408, 1365480013, 1365526203, …), we stopped trying to diagnose the npm-side trusted-publisher match and switched to a granular automation token with bypass-2FA, stored as the `NPM_TOKEN` GitHub Actions secret. *What we kept:* `--provenance` on every publish, so the SLSA attestation is still signed by GitHub's OIDC identity and recorded on Sigstore's public transparency log. Users verifying with `npm audit signatures` get the same cryptographic guarantee as with full-OIDC publishing. *What we lost:* the stored-token-free posture; a rotated laptop secret can now publish new versions. *Mitigations:* 90-day token rotation, scope limited to this single package, bypass-2FA enabled but no other permissions. *To revisit:* open an npm support ticket referencing the failing Rekor entries. Move back to OIDC auth when resolved.
...
## 2026-04-23 — Adopt phase-gated governance with explicit outcome metrics
Adopted a phased delivery model with explicit gates — threat model before detection logic, evaluation before release, ongoing drift monitoring after release. Each phase produces a named artefact. The shape was *inspired* by established industry frameworks for managing AI and data projects, but the project deliberately avoids adopting any framework's branding or vocabulary wholesale and states its discipline in its own terms. *Alternatives considered:* ad-hoc / "ship when tests pass". *Why rejected:* no gate for ethics, no drift story, no explicit value metrics.
